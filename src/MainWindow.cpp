...
//    ui->page_video->installEventFilter(this);
    ...
    ui->speed->setView(new QListView());
    // 初始化 speed ComboBox 控件的倍速选项
    ui->speed->addItem("0.5x", 0.5);
    ui->speed->addItem("0.75x", 0.75);
    ui->speed->addItem("1.0x", 1.0); // 默认速率
    ui->speed->addItem("1.25x", 1.25);
    ui->speed->addItem("1.5x", 1.5);
    ui->speed->addItem("2.0x", 2.0);

    // 设置默认选择项为1.0x
    ui->speed->setCurrentIndex(2);
    
    connect(ui->speed, QOverload<int>::of(&QComboBox::currentIndexChanged), this, [=](int index) {
        double speed = ui->speed->itemData(index).toDouble();
    ...
    mVolume = ui->horizontalSlider_volume->value();
...
{
    if (event->key() == Qt::Key_Up)  // 按上箭头键
    {
        mVolume += 5;  // 增加音量
        if (mVolume > 100) mVolume = 100;  // 限制最大音量为 100
        ui->horizontalSlider_volume->setValue(mVolume);  // 更新滑块值
    }
    else if (event->key() == Qt::Key_Down)  // 按下箭头键
    {
        mVolume -= 5;  // 减小音量
        if (mVolume < 0) mVolume = 0;  // 限制最小音量为 0
        ui->horizontalSlider_volume->setValue(mVolume);  // 更新滑块值
    }
    else if (event->key() == Qt::Key_Left || event->key() == Qt::Key_Right) {
        ui->horizontalSlider->keyPressEvent(event);  // 转发键盘事件到 VideoSlider
    }
